...
class AttachmentService extends Service {
    ...
    async create(payload) {
        return await this.ctx.model.Attachment.create(payload);
    }

    // destroy======================================================================================================>
    async destroy(_id) {
        const { ctx } = this;
        const attachment = await ctx.model.Attachment.find(_id);
        if (!attachment) {
        ...
        return await ctx.model.Attachment.findByIdAndRemove(_id);
    }
    ...
    async extra(_id, values) {
        ...
        return await this.ctx.model.Attachment.findByIdAndUpdate(_id, values);
    }
    ...
    // show======================================================================================================>
    async show(_id) {
        const attachment = await this.ctx.model.Attachment.find(_id);
        if (!attachment) {
            this.ctx.throw(404, 'attachment not found');
        }
        return await this.ctx.model.Attachment.findById(_id);
    }
        ...
        if (isPaging) {
            if (search) {
                if (kind) {
                    ...
                    res = await this.ctx.model.Attachment.find({ filename: { $regex: search } })
                        .skip(skip)
                        .limit(Number(pageSize))
                        .sort({ createdAt: -1 })
                        .exec();
                ...
                if (kind) {
                    res = await this.ctx.model.Attachment.find({ extname: { $in: attachmentKind[`${kind}`] } })
                        ...
                        .sort({ createdAt: -1 })
                        .exec();
                    count = await this.ctx.model.Attachment.count({ extname: { $in: attachmentKind[`${kind}`] } })
                        .exec();
                } else {
                    res = await this.ctx.model.Attachment.find({})
                        .skip(skip)
    ...
    // Commons======================================================================================================>
    async find(id) {
        return await this.ctx.model.Attachment.findById(id);
    }
}
