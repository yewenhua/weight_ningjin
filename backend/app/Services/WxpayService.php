...
class WxpayService
{
     ...
     * @param $desc
     */
    public function transfers($appid, $openid, $true_name='', $amount, $desc){
        $this->companyPayParameters = array();
        $this->companyPayParameters['mch_appid']  = $appid;
        ...
        $res = false;
        if(isset($rtn['return_code']) && $rtn['return_code'] == 'SUCCESS' && isset($rtn['result_code']) && $rtn['result_code'] == 'SUCCESS'){
            $res = true;
        }
        else{
            $res = false;
     ...
     * 查询企业付款接口
     */
    public function getTransferInfo($appid, $partner_trade_no){
        //最终返回值
        $this->companyPayParameters = array();
        $this->companyPayParameters['appid']  = $appid;
     ...
     */
    public function prepay($appid, $orderid, $openid, $type, $fee) {
        $this->publicPayParameters = array();
        $this->setPayParameters('appid', $appid);//公众账号ID
     ...
     * @param $out_trade_no
     */
    public function orderquery($appid, $transaction_id, $out_trade_no) {
        $this->publicPayParameters = array();
        $this->setPayParameters('appid', $appid);//公众账号ID
        $this->setPayParameters('mch_id', config('wxpay.mchid'));//商户号
        $this->setPayParameters('nonce_str', $this->createNoncestr());//随机字符串
     ...
     */
    public function closeorder($appid, $out_trade_no) {
        $this->publicPayParameters = array();
        $this->setPayParameters('appid', $appid);//公众账号ID
        $this->setPayParameters('mch_id', config('wxpay.mchid'));//商户号
        $this->setPayParameters('nonce_str', $this->createNoncestr());//随机字符串
     ...
     * @param $refund_fee
     */
    public function refund($appid, $transaction_id, $out_trade_no, $out_refund_no, $total_fee, $refund_fee) {
        $this->publicPayParameters = array();
        $this->setPayParameters('appid', $appid);//公众账号ID
        $this->setPayParameters('mch_id', config('wxpay.mchid'));//商户号
        $this->setPayParameters('nonce_str', $this->createNoncestr());//随机字符串
     ...
     * @param $refund_id
     */
    public function refundquery($appid, $transaction_id, $out_trade_no='', $out_refund_no='', $refund_id='') {
        $this->publicPayParameters = array();
        $this->setPayParameters('appid', $appid);//公众账号ID
        $this->setPayParameters('mch_id', config('wxpay.mchid'));//商户号
        $this->setPayParameters('nonce_str', $this->createNoncestr());//随机字符串
     ...
     * 下载对账单
     */
    public function downloadbill($appid) {
        $this->publicPayParameters = array();
        $this->setPayParameters('appid', $appid);//公众账号ID
        $this->setPayParameters('mch_id', config('wxpay.mchid'));//商户号
        $this->setPayParameters('nonce_str', $this->createNoncestr());//随机字符串
     ...
     * @param $long_url
     */
    public function getShortUrl($appid, $long_url)
    {
     ...
     * @param $product_id
     */
    public function createQrcodeUrl($appid, $product_id) {
        $time_stamp = time();
        if($product_id == null)
        {
            die("缺少Native支付二维码链接必填参数product_id！"."<br>");
        }
        $this->setScanParameters('appid', $appid);
    ...
    public function native_call() {
        ...
        $callbackData = $this->xmlToArray($postXML);
        if($this->checkSign($callbackData) != FALSE){
            $appid = $callbackData['appid']; //appid根据openid信息获取
            $product_id = $callbackData['product_id'];
            $openid = $callbackData['openid'];
            $type = 'NATIVE';
            $fee = 1;
            $rtn = $this->prepay($appid, $callbackData['out_trade_no'], $openid, $type, $fee);
            if($rtn != null){
                ...
                $this->setReturnParameters('appid', $appid);
                $this->setReturnParameters('mch_id', $callbackData['mch_id']);
