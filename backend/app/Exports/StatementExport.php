...
class StatementExport implements FromCollection, WithEvents, WithStrictNullComparison, WithProperties
{
    private $author;
    private $sheetname;
    public $data;
    ...
    public function __construct($data, $author='猫小鱼', $sheetname='运行记录报表')
    {
        $this->data = $data;
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                //设置区域单元格垂直居中
                $event->sheet->getDelegate()->getStyle('A1:AZ1265')->getAlignment()->setVertical('center');
                //设置区域单元格水平居中
                $event->sheet->getDelegate()->getStyle('A1:AZ1265')->getAlignment()->setHorizontal('center');
                $event->sheet->getDelegate()->setTitle($this->sheetname);
                $event->sheet->getStyle("A3:AK3")->getAlignment()->setWrapText(TRUE);
